...
/**
 * Home Page Component (Original design with functional navigation)
 * Main landing page for unauthenticated users.
 * Features smooth scrolling for all navigation links.
 */
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            ...
            <div className="flex items-center space-x-4">
              ...
              <Button className="bg-blue-600 hover:bg-blue-700" asChild><Link href="/register">สมัครสมาชิก</Link></Button>
            </div>
          </div>
        </div>
      </header>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-8">
              <div className="space-y-4">
                <Badge className="bg-blue-100 text-blue-800 border-blue-200"><Zap className="h-3 w-3 mr-1" /> เทคโนโลยี AI ล่าสุด</Badge>
                <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-gray-900 dark:text-white leading-tight">
                  ยกระดับความปลอดภัย<span className="text-blue-600">บนท้องถนน</span><br />ด้วยการตรวจจับความเหนื่อยล้าเรียลไทม์
                </h1>
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <Button size="lg" variant="outline" className="border-blue-600 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 text-lg px-8 py-6 border-2" onClick={() => scrollToSection('features')}>
                  ดูข้อมูลเพิ่มเติม
                </Button>
              </div>
              <div className="grid grid-cols-3 gap-6 pt-8">
                <div className="text-center"><div className="text-2xl font-bold text-blue-600">99.5%</div><div className="text-sm text-gray-600 dark:text-gray-400">ความแม่นยำ</div></div>
                <div className="text-center"><div className="text-2xl font-bold text-green-600">24/7</div><div className="text-sm text-gray-600 dark:text-gray-400">ตรวจสอบต่อเนื่อง</div></div>
                <div className="text-center"><div className="text-2xl font-bold text-purple-600 flex items-center justify-center">{isLoadingStats ? <Loader2 className="h-5 w-5 animate-spin" /> : formatDeviceCount(deviceCount)}</div><div className="text-sm text-gray-600 dark:text-gray-400">{deviceCount === 1 ? "อุปกรณ์" : "ผู้ใช้งาน"}</div></div>
              </div>
            </div>
            {/* Dashboard Preview */}
            <div className="relative"><div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl overflow-hidden border border-gray-200 dark:border-gray-700"><div className="p-4 bg-blue-600 text-white flex items-center justify-between"><div className="flex items-center space-x-2"><Image src="/logo.png" alt="Logo" width={24} height={24} className="h-6 w-6 brightness-0 invert" /><span className="font-medium">Driver Fatigue Dashboard</span></div><div className="flex items-center space-x-2"><div className="w-2 h-2 bg-green-400 rounded-full"></div><span className="text-xs">เชื่อมต่อแล้ว</span></div></div><div className="p-6"><div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6 flex items-center"><AlertTriangle className="h-5 w-5 text-yellow-500 mr-3" /><div><div className="font-medium text-yellow-800">ตรวจพบอาการง่วง</div><div className="text-sm text-yellow-700">แนะนำให้หยุดพักหรือเปลี่ยนคนขับ</div></div></div><div className="grid grid-cols-2 gap-4 mb-6"><div className="bg-white border rounded-lg p-4 shadow-sm"><div className="flex items-center justify-between mb-2"><div className="text-sm font-medium text-gray-500">สถานะปัจจุบัน</div><Eye className="h-4 w-4 text-blue-500" /></div><div className="text-2xl font-bold text-gray-900">ง่วงนอน</div><div className="mt-2 h-2 bg-gray-200 rounded-full overflow-hidden"><div className="h-full bg-yellow-500 rounded-full" style={{ width: "65%" }}></div></div></div><div className="bg-white border rounded-lg p-4 shadow-sm"><div className="flex items-center justify-between mb-2"><div className="text-sm font-medium text-gray-500">คะแนนความปลอดภัย</div><Activity className="h-4 w-4 text-green-500" /></div><div className="text-2xl font-bold text-gray-900">65/100</div><div className="text-sm text-gray-500 mt-2">ต่ำกว่าระดับปกติ (80+)</div></div></div><div className="bg-white border rounded-lg p-4 shadow-sm"><div className="flex items-center justify-between mb-4"><div><div className="font-medium">แนวโน้มความเหนื่อยล้า</div><div className="text-xs text-gray-500 mt-1">สีเขียว: ปกติ, สีเหลือง: ระวัง, สีแดง: อันตราย</div></div><BarChart3 className="h-4 w-4 text-gray-400" /></div><div className="h-24 flex items-end space-x-1">{[40, 35, 50, 45, 60, 75, 65, 80, 60, 55, 70, 85].map((value, i) => (<div key={i} className={`w-full rounded-t ${value > 70 ? "bg-red-400" : value > 50 ? "bg-yellow-400" : "bg-green-400"}`} style={{ height: `${Math.max(value * 0.8, 20)}%` }} title={`ช่วงเวลา ${i + 1}: ${value > 70 ? "อันตราย" : value > 50 ? "ระวัง" : "ปกติ"}`}></div>)) }</div><div className="text-xs text-gray-400 mt-2 text-center">12 ช่วงเวลาล่าสุด (แต่ละช่วง 10 นาที)</div></div></div></div><div className="absolute -top-4 -right-4 bg-red-100 dark:bg-red-900 border border-red-300 rounded-lg p-3 shadow-lg animate-pulse"><div className="flex items-center space-x-2"><Bell className="h-4 w-4 text-red-600" /><span className="text-sm font-medium text-red-800 dark:text-red-200">แจ้งเตือนด่วน!</span></div></div></div>
          </div>
        </div>
      </section>
