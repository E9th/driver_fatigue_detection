...
import { AlertTriangle, Eye, Users, Activity, Clock, BarChart3, Search } from "lucide-react"
import { LoadingScreen } from "@/components/loading-screen"
import { database } from "@/lib/firebase"
import { getAllUsers, deleteUser } from "@/lib/admin-utils"
import { signOut } from "@/lib/auth"
import { ref as dbRef, get } from "firebase/database" // เปลี่ยนชื่อเพื่อไม่ conflict กับ React ref
import { useToast } from "@/hooks/use-toast"
import type { UserProfile, SystemStats } from "@/lib/types"
...
export const AdminMasterDashboard = forwardRef<any, AdminMasterDashboardProps>(({ onStatsUpdate }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    getCurrentStats: () => {
      ...
      return {
        ...
        totalYawns: stats.totalYawns,
        totalDrowsiness: stats.totalDrowsiness,
        totalAlerts: stats.totalAlerts,
        systemUptime: 99.5,
        riskDistribution: {
          safe: 0,
          warning: stats.totalYawns,
          danger: stats.totalDrowsiness,
          critical: stats.totalAlerts,
        },
  ...
  // Load initial data - ทำครั้งเดียวตอน mount
  useEffect(() => {
    const loadAllData = async () => {
      ...
      try {
        ...
        const [alertsSnapshot, devicesSnapshot] = await Promise.all([
          get(dbRef(database, "alerts")),
          get(dbRef(database, "devices")),
        ])
    ...
    loadAllData()
  }, [toast]) // dependency เฉพาะ toast

  // Calculate stats when data or date range changes
  useEffect(() => {
    if (loading) return
    ...
    setStats(newStats)
  }, [users, alerts, devices, dateRange.startDate, dateRange.endDate, loading]) // แยก dependency ชัดเจน

  // Notify parent about stats update - แยกออกมาเป็น useEffect ต่างหาก
  useEffect(() => {
    if (loading || !onStatsUpdate) return
    ...
    // เรียก onStatsUpdate เฉพาะเมื่อ stats เปลี่ยนจริงๆ
    onStatsUpdate(systemStats)
  }, [stats, loading]) // ไม่ใส่ onStatsUpdate ใน dependency เพื่อป้องกัน infinite loop
  ...
  const hourlyActivity = useCallback(() => {
    const hourlyData = Array(24)
      .fill(0)
      .map((_, i) => ({ hour: i, yawns: 0, drowsiness: 0, alerts: 0, activeDevices: 0 }))
    ...
    return hourlyData
  }, [alerts, dateRange.startDate, dateRange.endDate])
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <Card>
              <CardHeader>
                <CardTitle>กิจกรรมตามช่วงเวลา</CardTitle>
              </CardHeader>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.map((user) => {
                    const isActive =
                      devices[user.deviceId] &&
                      devices[user.deviceId].current_data &&
                      Date.now() - new Date(devices[user.deviceId].current_data!.timestamp).getTime() < 5 * 60 * 1000
                    return (
                      <TableRow key={user.uid}>
                        ...
                        <TableCell>
                          ...
                          {isActive && <Badge className="ml-2 bg-green-100 text-green-800">ออนไลน์</Badge>}
                        </TableCell>
                        <TableCell className="text-right">
                          <Button variant="ghost" size="sm" onClick={() => router.push(`/admin/dashboard/${user.uid}`)}>
                            แดชบอร์ด
                          </Button>
                          {user.role !== "admin" && (
                            <AlertDialog>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>คุณต้องการลบผู้ใช้ {user.fullName} หรือไม่?</AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel>ยกเลิก</AlertDialogCancel>
                                  <AlertDialogAction onClick={() => handleDeleteUser(user.uid)}>ยืนยัน</AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
                            </AlertDialog>
                          )}
...
AdminMasterDashboard.displayName = "AdminMasterDashboard"
